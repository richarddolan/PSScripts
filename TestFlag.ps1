...
$Files=import-csv jcacmb.csv
$SiteURL="https://jhcloud.sharepoint.com/"

foreach($file in $files){
$SourceFileURL=$($file.SourceFile)
$TargetFileURL=$($file.Destination)

#Call the function to Move the File
IF ($($file.Compare) -eq "MOVE"){write-host "Moving $SourceFileURL" <#MoveSPOFile $SiteURL $SourceFileURL $TargetFileURL#>}
}